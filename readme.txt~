...
This email service implement the following Functionality:
http round trip between both client and email service provider
validate input data including html validation
dynamically select provider based on response from the provider.
Use lightweight queryable storage to keep records of emails passing through the service
It is deployed at HeroKu and can be accessed via a simple test ui:
http://obscure-springs-9663.herokuapp.com/
This is also the url which accepts http jasn post.
...
Service provider
3 files handles the interaction with services providers. An abstract class, EmailService.py define apis that all providers need to have.  It has two subclass MailGunEmailServiceHttp and MandrillEmailServiceHttp. Each implements the apis.  

Engine

Email_Handler is the service engine. It is created before service starts to accept request. When it is created, it finds all the providers from a config file and put them in a list.  So every provider has to have one configuration entry in the config file in order to register. When engine reads the config file at the run time, it dynamically imports each provider’s api class. Such design pattern makes adding new service provider without changing the existing code.  Adding new provider only has 3 requirments.  First, new class needs to inherit abstract service class. Second, add an entry in the config file for new class needs. And the last, new class needs to be placed in the same directory where all other existing service apis sit so engine knows where to import it.
...
Whenever a request arrives,  engine tries to send the request to providers and stops if either sending is successful or it goes through the list and find no provider accepts the request. 

Validation

Data validation is implemented in email_message.py not in service providers’ api. Because data validations are  same for all providers. So it should be done by service broker, which is our service. Engine performs validation before calling providers api.  Here is the validation detail on each input form. 
Email validation uses regular expression.  The actual expression imitates jdango  email check. One decision here is not using other web service to check email address.  Because often one email can have a huge recipient list, including to, cc, bcc.  If everyone email address needs a http round trip to validate, performance can be really bad.  Also using webhook to get delivery feedback is a better solution.
